...
/**
 * Unified DNS-over-HTTPS handler extracted from Cloudflare Worker version.
 * This file stays platform-agnostic and exposes handleRequest(request, platform).
 * platform: { name: 'netlify' | 'vercel', getEnv: (k:string)=>string|undefined, getClientIP: (req:Request)=>string|undefined }
 */
...
let DoH路径 = 'dns-query';
export async function handleRequest(request, platform) {
    if (platform.getEnv("DOH")) {
      DoH = platform.getEnv("DOH");
      const match = DoH.match(/:\/\/([^\/]+)/);
      if (match) {
        DoH = match[1];
      }
    }
    DoH路径 = platform.getEnv("PATH") || platform.getEnv("TOKEN") || DoH路径;//DoH路径也单独设置 变量PATH
    ...
    // 添加IP地理位置信息查询代理
    if (path === '/ip-info') {
      if (platform.getEnv("TOKEN")) {
        const token = url.searchParams.get('token');
        if (token != platform.getEnv("TOKEN")) {
      ...
      const ip = url.searchParams.get('ip') || platform.getClientIP(request);
      if (!ip) {
        return new Response(JSON.stringify({ 
          status: "error",
          message: "IP参数未提供",
    ...
    if (platform.getEnv("URL")302) return Response.redirect(platform.getEnv("URL")302, 302);
    else if (platform.getEnv("URL")) {
      if (platform.getEnv("URL").toString().toLowerCase() == 'nginx') {
        return new Response(await nginx(), {
          headers: {
            'Content-Type': 'text/html; charset=UTF-8',
          },
        });
      } else return await 代理URL(platform.getEnv("URL"), url);
